...
class TfIdf:
    # Sentences Tokenize
    def _sentences(self, sentences):
        #rake_nltk_var = Rake()
        #rake_nltk_var.extract_keywords_from_text(sentences)
        #keyword_extracted = rake_nltk_var.get_ranked_phrases()
        #sentences = re.sub(r'[^a-zA-Z.\d\s]', '', str(sentences))
        sentences = str(sentences).lower()
        sentences = sentences.strip()
        return sent_tokenize(sentences)

    # Total Documents
    def _total_documents(self, sentences):
        return len(sentences)
     # Total Documents

    #Get Result
    def _results_update(self, matrix, keyName, results):
        for t in matrix:
            if(matrix[t]):
                #print(tf_matrix[tfs])
                for tKey, tVal in matrix[t].items():
                    if(not tKey.isalpha()):
                        continue
                    #print(isinstance(results[tfidfKey], dict))
                    #print(results[tfidfKey] is dict)
                    #print(f"{tKey} : {tVal}")
                    if( isinstance(results[tKey], dict) ):
                        results[tKey].update({keyName: round(tVal, 2)})
    ...
    def _create_frequency_table(self,text_string) -> dict:
        ...
        newStopWords = get_stopwords.allStopWords
    ...
    def _score_sentences(self, tf_idf_matrix) -> dict:
        ...
        for sent, f_table in tf_idf_matrix.items():
            ...
            sentenceValue[sent] = round(total_score_per_sentence / \
                count_words_in_sentence,2)
